...
if ($nbtot > 0) {
    ...
    foreach ($visible_rows as $data) {
        ...
        echo Tool::showItem($output_type, Tool::escapeForOutput($output_type, Dropdown::getDropdownName('glpi_entities', $data['entities_id'])), $num, $row_num);
        echo Tool::showItem($output_type, Ticket::getStatus($data["status"]), $num, $row_num);
        echo Tool::showItem($output_type, Html::convDateTime($data['date']), $num, $row_num);
        echo Tool::showItem($output_type, Html::convDateTime($data['date_mod']), $num, $row_num);
        ...
        if ($output_type == Search::HTML_OUTPUT
            ...
            echo Tool::showItem($output_type, Html::timestampToString($data["takeintoaccount_delay_stat"]), $num, $row_num);
        } else {
            echo Tool::showItem($output_type, convertTimestamp($data["takeintoaccount_delay_stat"]), $num, $row_num);
        }
