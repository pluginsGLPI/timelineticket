...
$report = new AutoReport(__("statResolvedSpentTimeByGroup_report_title", "timelineticket"));
...
$date = new DateIntervalCriteria($report, '`glpi_tickets`.`solvedate`', __('Date of solving'));
...
$order_criteria = getOrderByCriteria('solvedate', $columns);
if ($order_criteria !== []) {
...
if ($nbtot > 0) {
    ...
    if (!empty($mylevels)) {
        foreach ($mylevels as $key => $val) {
            ...
            showTitle($output_type, $num, __('Duration by', 'timelineticket') . "&nbsp;" . Tool::escapeForOutput($output_type, $key), '', false);
        }
    }
    ...
    foreach ($visible_rows as $data) {
        ...
        foreach ($ticket->getGroups(CommonITILActor::ASSIGN) as $current_group) {
            // The column is deliberately HTML (names are joined with <br>), so each name
            // has to be escaped individually rather than the whole string at the sink.
            $groups .= Tool::escapeForOutput($output_type, Dropdown::getDropdownName("glpi_groups", $current_group["groups_id"]));
            $groups .= "<br>";
        }
